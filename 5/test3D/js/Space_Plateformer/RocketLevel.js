...
export default class RocketLevel extends Phaser.Scene {
  ...
  create() {
    ...
    this.platform = this.physics.add.image(350, 1300, 'platform_image').setScale(1);
    ...
    this.rocket = this.physics.add.sprite(550, 850, 'rocket_idle').setScale(0.3);
  ...
  startBlockGeneration() {
    ...
    this.blockEvent = this.time.addEvent({
      delay: 1000,
      callback: () => {
        const block = this.blocks.create(1920, Phaser.Math.Between(100, 980), 'block').setScale(0.5);
        const speedX = -250;
  ...
  createLasers() {
    ...
    for (let i = 0; i < laserCount; i++) {
      ...
      const laser = this.add.rectangle(0, laserY, 1920, 30, 0xff0000).setOrigin(0, 0.5);
      laser.setAlpha(0.5);
  ...
  createEnemies() {
    ...
    for (let i = 0; i < enemyCount; i++) {
      ...
      const enemy = this.physics.add.sprite(1700, enemyY, 'enemy_space').setScale(0.5);
      ...
      this.physics.add.overlap(this.playerProjectiles, enemy, this.hitEnemy, null, this);
    }
  }

  shootProjectile(enemy) {
    const projectile = this.projectiles.create(enemy.x, enemy.y, 'bossProjectile').setScale(0.3);
    projectile.setVelocityX(-400);
    projectile.body.allowGravity = false;
    projectile.setCollideWorldBounds(false);

    this.physics.add.overlap(this.rocket, projectile, this.hitProjectile, null, this);
  }
  ...
  shootPlayerProjectile() {
    const projectile = this.playerProjectiles.create(this.rocket.x, this.rocket.y, 'playerProjectile').setScale(0.5);
  ...
  startLifeGeneration() {
    this.lifeEvent = this.time.addEvent({
      ...
      callback: () => {
        ...
        this.physics.add.overlap(this.rocket, life, this.collectLife, null, this);
      },
      loop: true
    });
  }
  ...
  update() {
    if (!this.rocketControlled) {
      ...
      if (distanceToRocket < 50 && this.cursors.enter.isDown) {
        this.enterRocket();
      }
    } else {
      ...
      this.physics.world.collide(this.rocket, this.blocks, this.hitBlock, null, this);
    }
  }
