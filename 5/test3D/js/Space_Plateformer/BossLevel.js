...
export default class BossLevel extends Phaser.Scene {
  ...
  create() {
    ...
    this.rocket = this.physics.add.sprite(400, 540, 'rocket_active').setScale(0.3);
    ...
    this.cursors = this.input.keyboard.addKeys({
      ...
      basicShoot: Phaser.Input.Keyboard.KeyCodes.I,
      arcShoot: Phaser.Input.Keyboard.KeyCodes.O,
    ...
    this.physics.add.overlap(this.boss, this.playerProjectiles, this.hitBoss, null, this);
  ...
  createAttackIcons() {
    ...
    icons.forEach((icon, index) => {
      ...
      const background = this.add.rectangle(x, 1020, 70, 70, 0x000000).setOrigin(0.5).setAlpha(0.5);
      const image = this.add.image(x, 1020, icon).setScale(0.2);
      const cooldownOverlay = this.add.rectangle(x, 1020, 68, 68, 0xff0000).setOrigin(0.5).setAlpha(0);
  ...
  updateAttackCooldowns(time) {
    ...
    Object.keys(this.attackIcons).forEach((key) => {
      ...
      const cooldownKey = key.replace('playerProjectile', 'basic').replace('arcProjectile', 'arc').replace('laser', 'laser');
      const cooldown = cooldowns[key];

      const remainingTime = Math.max(0, (this.cooldowns[cooldownKey] - time) / cooldown);
  ...
  shootRandomProjectiles() {
    for (let i = 0; i < 7; i++) {
      ...
      const projectile = this.physics.add.sprite(this.boss.x, this.boss.y, 'bossProjectile').setScale(0.3);
      projectile.body.allowGravity = false;
      projectile.setVelocity(Math.cos(angle) * speed, Math.sin(angle) * speed);
      projectile.rotation = angle;
  
      // Ajouter la détection de collision entre les projectiles et la fusée
      this.physics.add.overlap(this.rocket, projectile, () => {
  ...
  spawnBombs() {
    ...
    for (let i = 0; i < bombCount; i++) {
      ...
      } while (Phaser.Math.Distance.Between(bombX, bombY, this.boss.x, this.boss.y) < 200);

      const bomb = this.add.image(bombX, bombY, 'bomb').setScale(0.5);
      const bombCenter = this.add.image(bombX, bombY, 'bomb_center').setScale(0.05);
      bomb.setAlpha(0.5);
      ...
      this.time.delayedCall(1000, () => {
        ...
        const explosionRadius = this.add.circle(bomb.x, bomb.y, 100, 0xff0000, 0.5);
        this.physics.add.existing(explosionRadius, true);

        this.time.delayedCall(1000, () => {
          explosionRadius.setFillStyle(0xff0000, 1);

          this.physics.add.overlap(this.rocket, explosionRadius, () => {
            this.player.decreaseHealth();
            bomb.destroy();
          }, null, this);
  ...
  if (this.rocket.x < this.cameras.main.width / 2) {
    ...
    if (this.cursors.basicShoot.isDown && time > this.cooldowns.basic) {
      this.shootPlayerProjectile();
      this.cooldowns.basic = time + 500;
    }
  
    if (this.cursors.arcShoot.isDown && time > this.cooldowns.arc) {
  ...
  shootPlayerProjectile() {
    const projectile = this.playerProjectiles.create(this.rocket.x, this.rocket.y, 'playerProjectile').setScale(0.3);
    projectile.body.allowGravity = false;
  
    // Calcul de la direction du tir vers le boss
    const angle = Phaser.Math.Angle.Between(projectile.x, projectile.y, this.boss.x, this.boss.y);
    const speed = 300;
  ...
  shootArcProjectile() {
    ...
    arcAngles.forEach((offset) => {
      const projectile = this.playerProjectiles.create(this.rocket.x, this.rocket.y, 'arcProjectile').setScale(0.3);
      projectile.body.allowGravity = false;

      const angle = Phaser.Math.Angle.Between(projectile.x, projectile.y, this.boss.x, this.boss.y) + offset;
      const speed = 300;
      projectile.setVelocity(Math.cos(angle) * speed, Math.sin(angle) * speed);
      projectile.rotation = angle;
    });
  ...
  shootLaser() {
    ...
    const angle = Phaser.Math.Angle.Between(this.rocket.x, this.rocket.y, this.boss.x, this.boss.y);
    const laserLength = 1000;
    const laserThickness = 5;
    const laser = this.add.rectangle(this.rocket.x, this.rocket.y, laserLength, laserThickness, 0xff0000).setOrigin(0, 0.5);
    laser.setAlpha(0.5);
  ...
  shootLaserFromBoss() {
    ...
    for (let i = 0; i < numHorizontalLasers; i++) {
      ...
      const laser = this.add.rectangle(0, yPosition, screenWidth, 5, 0xff0000).setOrigin(0, 0.5);
    ...
    for (let i = 0; i < numVerticalLasers; i++) {
      ...
      const laser = this.add.rectangle(xPosition, 0, 5, screenHeight, 0xff0000).setOrigin(0.5, 0);
      laser.setAlpha(0.5); // Opacité initiale à 0.5
      this.physics.add.existing(laser, true);
  ...
  hitBoss(boss, projectile) {
    ...
    if (this.bossHealth <= 0) {
      this.bossDefeated();
    } else if (this.bossHealth <= this.bossMaxHealth * 0.6 && !this.phaseTwoStarted) {
      this.phaseTwoStarted = true;
      this.startPhaseTwo();
    } else if (this.bossHealth <= this.bossMaxHealth * 0.3 && !this.phaseThreeStarted) {
  ...
  this.chest = this.physics.add.sprite(1800, 400, 'chest_close').setScale(0.2);
  ...
  this.portal.body.allowGravity = false;
   ...
   this.physics.add.overlap(this.rocket , this.chest, this.attemptOpenChest, null, this);
   this.physics.add.overlap(this.rocket, this.portal, this.attemptEnterPortal, null, this);
  }
