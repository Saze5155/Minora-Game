...
export default class Laby_map1 extends Phaser.Scene {
  constructor() {
    super({ key: "Laby_map1" });
  ...
  create() {
    ...
    this.map = this.make.tilemap({ key: "laby_map1" });
    ...
    enemyPoints.forEach((point) => {
      ...
      enemy.sprite.enemyInstance = enemy;
      this.enemyGroups.add(enemy.sprite); // Ajoute le sprite à enemyGroups pour les collisions
    });

    const returnZone = this.object_layer.objects.find(
      (obj) => obj.name === "monde"
    );

    if (returnZone) {
      const zone = this.physics.add.staticImage(
        returnZone.x + returnZone.width / 2,
        returnZone.y + returnZone.height / 2
      );
      zone.setSize(returnZone.width, returnZone.height);
      zone.setVisible(false); // Rendre la zone invisible
  ...
  collectReward() {
    ...
    if (!this.competence) {
      ...
      const newAttack = Global.attacks["temps"][0];
  ...
  sortie() {
    if (this.player.keys.interact.isDown) {
      this.scene.stop("Laby_map1");

      this.scene.start("Laby_map2");
    }
  }
