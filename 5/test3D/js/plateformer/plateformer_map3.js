...
export default class Plateformer_map3 extends Phaser.Scene {
  constructor() {
    super({ key: "Plateformer_map3" });
  ...
  create() {
    ...
    this.porte = this.physics.add.image(porte.x, porte.y, "porte_boss");
    this.porte.setScale(15);
  ...
  update() {
    ...
    if (this.player.sprite.y < 31382 && this.death_layer.y > -29720) {
      this.death_layer.y -= 8.5; // Le death layer descend progressivement
    }
  ...
  coffreOuverture(scene) {
    if (this.player.keys.interact.isDown && !this.isCoffreOuvert) {
      ...
      const newAttack = Global.attacks["nature"][2];
  ...
  sortie() {
    if (this.player.keys.interact.isDown) {
      this.scene.stop("Plateformer_map3");
      Global.enemyId = 1;
