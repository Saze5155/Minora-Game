...
export default class Plateformer_map1 extends Phaser.Scene {
  constructor() {
    super({ key: "Plateformer_map1" });
  ...
  coffreOuverture(scene) {
    if (this.player.keys.interact.isDown && !this.isCoffreOuvert) {
      ...
      const newAttack = Global.attacks["nature"][0];
  ...
  sortie() {
    if (this.player.keys.interact.isDown) {
      ...
      this.scene.start("Plateformer_map2");
    }
  }
