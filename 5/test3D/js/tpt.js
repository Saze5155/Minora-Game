...
export default class tpt extends Scene3D {
  ...
  create() {
    ...
    this.attackButton = this.add
      .image(200, 500, "attackButton")
      .setInteractive();
    this.defendButton = this.add
      .image(350, 500, "defendButton")
      .setInteractive();
    this.potionButton = this.add
      .image(500, 500, "potionButton")
      .setInteractive();
    ...
    this.attackMenu = this.add
      .text(400, 500, "", { fontSize: "16px", fill: "#fff" })
      .setVisible(false);
  ...
  usePotion(type) {
    if (type === "health") {
      this.playerHP = Math.min(
        this.playerHP + playerConfig.potions.healAmount,
        100
      );
      this.updateHealthBar(this.playerHealthBar, this.playerHP, 100);
      this.updatePlayerActionText(
        `Le joueur récupère ${playerConfig.potions.healAmount} HP.`
      );
    } else if (type === "mana") {
      this.playerMP = Math.min(
        this.playerMP + playerConfig.potions.manaAmount,
        50
      );
      this.playerMPText.setText(`MP: ${this.playerMP}`);
      this.updatePlayerActionText(
        `Le joueur récupère ${playerConfig.potions.manaAmount} MP.`
      );
    }
