...
export default class tpt extends Phaser.Scene {
  ...
  showAttackMenu() {
    ...
    const startY =
      this.scale.height / 2 -
      ((slotSize + padding) * Global.attackActif.length) / 2; // Centrer verticalement
    ...
    Global.attackActif.forEach((attack, index) => {
      ...
      const attackImage = this.add
        .image(startX, y, attack.image)
        .setInteractive();
      ...
      attackImage.on("pointerover", () => {
        attackImage.setScale(0.1).setDepth(502); // Agrandir légèrement au survol
      });
    ...
    this.returnButton = this.add
      .image(this.scale.width / 2, this.scale.height - 50, "returnButton")
      .setInteractive();
  ...
  showPotionMenu() {
    ...
    potionImages.forEach((potion, index) => {
      const potionImage = this.add
        .image(startX + index * spacing, startY, potion)
        .setInteractive();
      ...
      const potionText = this.add
        .text(startX + index * spacing, startY - 50, "", {
          fontSize: "20px",
          fill: "#fff",
          fontStyle: "bold",
          align: "center",
        ...
        .setOrigin(0.5)
        .setVisible(false); // Masquer le texte initialement
    ...
    this.returnButton = this.add
      .image(this.scale.width - 50, this.scale.height - 100, "returnButton")
      .setInteractive();
    this.returnButton.setScale(0.5); // Ajuster la taille si nécessaire
    ...
    if (this.playerMP >= attack.mpCost) {
      ...
      if (this.forceBoost) {
        ...
        this.updatePlayerActionText(
          `Votre attaque est boostée par ${this.forceBoost} points de force !`
        );
      ...
      this.updatePlayerActionText(
        `${attack.name} inflige ${damage} dégâts à l'ennemi !`
      );
  ...
  playerDefend() {
    ...
    this.updatePlayerActionText(
      `Le joueur se défend et réduit les dégâts de ${this.defenseBoost} points.`
    );
    this.endPlayerTurn();
  }
  ...
  usePotion(potionType) {
    ...
    if (
      ...
      this.updatePlayerActionText(
        "Défense augmentée, vous subirez moins de dégâts pendant un tour !"
      );
      ...
      if (this.enemyHP <= 0) {
        this.updatePlayerActionText(
          "L'ennemi a 0 HP après l'inversion, vous avez gagné !"
        );
        this.endCombat("win");
      }
    }
  ...
  enemyTurn() {
    ...
    this.time.delayedCall(1000, () => {
      ...
      if (
        this.enemyHP <= 30 &&
        Global.inventory.potions["vie"] &&
        Global.inventory.potions["vie"].length > 0 &&
        Math.random() < 0.2
      ) {
      ...
      if (
        this.enemyMP <= 10 &&
        Global.inventory.potions["mana"] &&
        Global.inventory.potions["mana"].length > 0 &&
        Math.random() < 0.2
      ) {
  ...
  enemyAttack() {
    ...
    if (this.defenseBoost) {
      ...
      this.updatePlayerActionText(
        `Vous avez bloqué ${this.defenseBoost} points de dégâts !`
      );
    }
    ...
    let message =
      damage > 15
        ? "L'ennemi a porté un coup critique !"
        : "L'ennemi a infligé des dégâts.";
